...
open OperatorKO7.KBOSubtermCoefficient (SubtermCoefficientKBO)
...
inductive PathOrderMechanism
  ...
  | coefficientWeightedVariableBarrier
...
def pathOrderClassifications : List PathOrderClassification :=
  ...
  , { family := .subtermCoefficientKBO
      status := .barrier
      mechanism := .coefficientWeightedVariableBarrier
      status_matches := rfl }
  , { family := .acKBO
      status := .barrier
  ...
  , { family := .popStarFamily
      status := .barrier
      mechanism := .popSafeArgumentBarrier
      status_matches := rfl }
  , { family := .simpleTerminationOrderType
...
/-- SUPERSEDED. Typed hypotheses that were used to classify the
subterm-coefficient KBO row as a conditional escape. The record counts payload
occurrences (`rhsPayloadWeight = 2`) instead of counting them weighted by the
argument coefficients along the path. Under the coefficient-weighted count the
right-hand side carries `wrap₁ + wrap₂ * recur₂`, which is at least
`1 + recur₂` because every argument coefficient is at least one, so
`coefficient_covers_duplication` cannot be met by any coefficient assignment.
The row is now classified by `subtermCoefficientKBO_barrier`. This record is
retained because the reach gate pins it; it backs no classification. -/
...
/-- The unweighted record above is unreachable from a genuine subterm-coefficient
assignment: read at the payload variable, its `coefficient_covers_duplication`
field asserts exactly what the weighted count refutes. -/
theorem subtermCoefficientKBOHypotheses_not_from_weighted_counts
    (C : SubtermCoefficients) (h : SubtermCoefficientKBOHypotheses)
    (hlhs : h.lhsPayloadWeight =
      weightedCount C.toCoefficientAssignment SchemaVar.s dupSrc)
    (hrhs : h.rhsPayloadWeight =
      weightedCount C.toCoefficientAssignment SchemaVar.s dupTgt) :
    False := by
  have hcov := h.coefficient_covers_duplication
  have hlt := weightedCount_dup_payload_strict C
  omega

/-- No admissible KBO with subterm coefficients orients the duplicating rule,
whatever the coefficients. -/
theorem subtermCoefficientKBO_no_weighted_orientation :
    ¬ ∃ (C : SubtermCoefficients) (O : WeightedVariableConditionOrder C),
        O.gt dupSrc dupTgt :=
  no_subtermCoefficient_variable_condition_orients_dup_step

/-- Carrier-level form: every admissible subterm-coefficient KBO on the schema
signature fails at the actual KO7 recursor-successor root step. -/
theorem subtermCoefficientKBO_carrier_blocked
  ...
  OperatorKO7.KBOSubtermCoefficient.subtermCoefficientKBO_no_ko7_rec_succ K

/-- The subterm-coefficient KBO row is a barrier. The status value and the
obstruction that justifies it are stated together so the row cannot drift from
its mechanism. -/
theorem subtermCoefficientKBO_barrier :
    statusOf .subtermCoefficientKBO = .barrier
      ∧ ¬ ∃ (C : SubtermCoefficients) (O : WeightedVariableConditionOrder C),
          O.gt dupSrc dupTgt :=
  ⟨rfl, subtermCoefficientKBO_no_weighted_orientation⟩
...
theorem popStarFamily_safe_duplication_barrier
    (_variant : POPStarVariant) (_h : POPStarSafeArgumentHypotheses) :
    statusOf .popStarFamily = .barrier := by
  rfl
...
structure PathOrderLayerClosed where
  ...
  kbo_rows_blocked :
    ∀ _ : KBOBarrierVariant,
      ¬ ∃ K : OperatorKO7.KBOImpossible.KBOStyleOrder, K.gt dupSrc dupTgt
  subterm_coefficient_row_blocked :
    statusOf .subtermCoefficientKBO = .barrier
      ∧ ¬ ∃ (C : SubtermCoefficients) (O : WeightedVariableConditionOrder C),
          O.gt dupSrc dupTgt
  subterm_coefficient_carrier_blocked :
    ∀ K : OperatorKO7.KBOSubtermCoefficient.SubtermCoefficientKBO,
      OperatorKO7.KBOSubtermCoefficient.SubtermCoefficientKBORecSuccObstruction K
  ...
  pop_status : statusOf .popStarFamily = .barrier
...
theorem rdrs_path_order_layer_closed : PathOrderLayerClosed where
  ...
  subterm_coefficient_row_blocked := subtermCoefficientKBO_barrier
  subterm_coefficient_carrier_blocked := subtermCoefficientKBO_carrier_blocked
