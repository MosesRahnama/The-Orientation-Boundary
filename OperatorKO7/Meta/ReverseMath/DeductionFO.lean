...
inductive DerivableFO (T : L.Theory) : {n : ℕ} → L.BoundedFormula Empty n → Prop
  ...
  | all_elim {n : ℕ} {φ : L.BoundedFormula Empty (n + 1)} (h : DerivableFO T φ.all) :
      DerivableFO T φ
  /-- **Free-variable weakening**: add one unused highest-index open variable.  This is the
  structural rule needed to reuse a universally valid open derivation inside a larger open
  context. -/
  | freeVarWeakening {n : ℕ} {φ : L.BoundedFormula Empty n} (h : DerivableFO T φ) :
      DerivableFO T (φ.liftAt 1 n)
...
theorem derivableFO_sound {M : Type*} [L.Structure M] [Nonempty M] {T : L.Theory}
  ...
  | freeVarWeakening _ ih =>
      intro env
      rw [BoundedFormula.realize_liftAt_one_self]
      exact ih (env ∘ Fin.castSucc)
  | spec t _ ih =>
      intro env
