...
theorem artsGieslSctSoundness_faithful :
  ...
  simp only [ArtsGieslSctSoundnessFormula, numberPredecessorDescentSentence,
    numberPredecessorDescentMatrix, ActualArtsGieslSctSoundness, ltBd, isSetBd,
...
/-- The canonical name for the arithmetical content of the predecessor-descent sentence: every
natural number has a strictly smaller descent value unless it is `0`. -/
def ActualNumberPredecessorDescent : Prop := ActualArtsGieslSctSoundness

/-- The canonical-name form of standard-model faithfulness for the predecessor-descent sentence. -/
theorem numberPredecessorDescent_faithful :
    (StdCarrier ⊨ numberPredecessorDescentSentence) ↔ ActualNumberPredecessorDescent :=
  artsGieslSctSoundness_faithful

/-- The canonical-name form of the arithmetical content, proved over `ℕ`. -/
theorem actualNumberPredecessorDescent_holds : ActualNumberPredecessorDescent :=
  actualArtsGieslSctSoundness_holds

/-- The standard model satisfies the predecessor-descent sentence, under its canonical name. -/
theorem stdModel_models_numberPredecessorDescent :
    StdCarrier ⊨ numberPredecessorDescentSentence :=
  stdModel_models_artsGieslSctSoundness
