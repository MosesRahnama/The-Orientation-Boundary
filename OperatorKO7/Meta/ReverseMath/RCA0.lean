...
/-- Accurate canonical name for the finite basic-arithmetic fragment `rca0BasicAxioms`: it contains
the induction-needing axioms `axZeroOrSucc` and `axLtIrrefl`, and it is not full `RCA₀` (it has no
comprehension scheme and no induction scheme). -/
def finitePredecessorArithmeticAxioms : L2.Theory := rca0BasicAxioms

/-- The standard model satisfies the finite predecessor-arithmetic fragment, under its canonical
name. -/
theorem stdModel_models_finitePredecessorArithmeticAxioms :
    StdCarrier ⊨ finitePredecessorArithmeticAxioms :=
  stdModel_models_rca0BasicAxioms
