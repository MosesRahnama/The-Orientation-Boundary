...
private theorem contra {T : L2.Theory} {n : ℕ} {Γ : List (L2.BoundedFormula Empty n)}
    {A C : L2.BoundedFormula Empty n} (hA : DerivableH T Γ A)
    (hnA : DerivableH T Γ A.not) : DerivableH T Γ C := by
  have h : DerivableH T Γ (A.imp ⊥) := hnA
...
private theorem lemmaA {T : L2.Theory} :
    DerivableFO T ((isSetBd (&0)) ⟹ (∃' sctMatrix)) := by
  show DerivableH T [] ((isSetBd (&0)) ⟹ (∃' sctMatrix))
  apply deduction
...
private theorem lemmaB {T : L2.Theory} :
    DerivableFO T ((Term.bdEqual (&0) zeroTerm) ⟹ (∃' sctMatrix)) := by
  show DerivableH T [] ((Term.bdEqual (&0) zeroTerm) ⟹ (∃' sctMatrix))
  apply deduction
  apply ex_intro_H (&0 : L2.Term (Empty ⊕ Fin 1))
  rw [instTop_self_sctMatrix]
  apply deduction
...
private theorem lemmaC {T : L2.Theory} :
    DerivableFO T ((∃' zosSucc) ⟹ (∃' sctMatrix)) := by
  apply DerivableFO.ex_mono
  apply DerivableFO.all_intro
  show DerivableH T [] (zosSucc.imp sctMatrix)
  apply deduction
  apply deduction
  have hP : DerivableH T [zosSucc, (isSetBd (&0)).not] zosSucc :=
    weaken (assume_last [] zosSucc)
  exact and_intro_H (and_left_H hP) (or_inl_H (and_right_H (and_right_H hP)))

/-! ### The object derivation -/

/-- Any first-order theory that derives zero-or-successor also derives the elementary
predecessor-descent sentence.  No other arithmetic axiom is used. -/
theorem numberPredecessorDescent_derivable_of_axZeroOrSucc {T : L2.Theory}
    (hZOS : DerivableFO T axZeroOrSucc) :
    DerivableFO T numberPredecessorDescentSentence := by
  change DerivableFO T (∀' ∃' sctMatrix)
  apply DerivableFO.all_intro
  show DerivableH T [] (∃' sctMatrix)
  refine or_elim_H (em_H (isSetBd (&0))) ?_ ?_
  · exact mp_H (ofClosed (lemmaA (T := T))) (assume_last [] (isSetBd (&0)))
  · have hZOSbody : DerivableFO T zosBody :=
      DerivableFO.all_elim (φ := zosBody) hZOS
    have hdisj : DerivableH T [(isSetBd (&0)).not]
        ((Term.bdEqual (&0) zeroTerm) ⊔ ∃' zosSucc) :=
      mp_H (ofClosed hZOSbody) (assume_last [] ((isSetBd (&0)).not))
    refine or_elim_H hdisj ?_ ?_
    · exact mp_H (ofClosed (lemmaB (T := T)))
        (assume_last [(isSetBd (&0)).not] (Term.bdEqual (&0) zeroTerm))
    · exact mp_H (ofClosed (lemmaC (T := T)))
...
calculus.  The historical name is retained for compatibility; this theorem does not formalize the
Arts-Giesl dependency-pair theorem. -/
theorem artsGiesl_syntactic_upper :
    DerivableFO rca0BasicAxioms ArtsGieslSctSoundnessFormula := by
  exact numberPredecessorDescent_derivable_of_axZeroOrSucc
