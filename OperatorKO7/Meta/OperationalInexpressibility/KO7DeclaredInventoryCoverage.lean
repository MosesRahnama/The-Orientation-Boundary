...
theorem atlasClassification_exact_counts :
    payloadSensitiveBlockedFamilies.length = 28 ∧
      projectionTransactionEscapeFamilies.length = 8 ∧
      constructionEscapeFamilies.length = 7 ∧
...
structure PaperADeclaredInventoryClosed : Prop where
  ...
  projectionTransactionEscapeCount :
    projectionTransactionEscapeFamilies.length = 8
  constructionEscapeCount : constructionEscapeFamilies.length = 7
