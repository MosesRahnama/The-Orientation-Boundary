...
/-!
# Matrix Unrestricted Split -- Acyclic Core
...
This acyclic core is theorem-only; no `axiom`, no `sorry`. Universal-closure
...
`Meta/BoundaryOperator/*`, or `Meta/MatrixBarrier{Arbitrary,ArcticTropical}*`
file is modified.
-/
...
/-- Core arctic payload, stated directly against the certificate-backed
matrix theorem so the unrestricted split has no catalog dependency. -/
...
/-- Core proof of the arctic payload. -/
...
/-- Core proof of the tropical payload. -/
