...
## Tier note: where the content lives

This file is the **attestation tier**. Its five evidence fields are whatever
proposition the caller chooses, so a `SemanticDirectMeasure` on its own is not
evidence that the measure avoids a rewrite oracle, a transformed relation, a
semantic quotient, a DP processor, or an external proof artifact. Read the
five projections (`NoRewriteOracle`, `NoTransformedRelation`,
`NoArbitrarySemanticQuotient`, `NoDPProcessor`, `NoExternalProofLanguage`) and
`DirectConstructorLaw_holds` accordingly: they return the stored proposition,
whatever it is, and `natConstantDirectMeasure` shows that `True` is admissible.

`Meta/RDRSMechanicalDirectness.lean` supplies the **mechanical tier** that does
carry content. There a measure is closed `NatMeasureExpr` code with no `Prop`
attestation field at all, and the five evidence propositions are instantiated
with proved route-nondependence statements over an explicitly modeled
`RouteEnvironment`. What those statements establish is closed-code and
extensional independence from five modeled explicit route inputs; they do not
establish historical provenance of an arbitrary function's construction, and
route inputs outside the five modeled slots stay outside the claim. The
mechanical image is proved strictly smaller than this tier: a non-monotone
payload measure carries the very same five nondependence proofs and is still
not the evaluation of any reflected code
(`nonMonotoneSemanticDirectMeasure_not_inMechanicalImage`).

Import direction is one-way: the mechanical file imports the reflected DSL and,
through it, this file. Nothing is imported back here.
