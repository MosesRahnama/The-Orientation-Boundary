...
/-- **(Determinacy of the KO7 diagonal.)** Under the full kernel `Step`, the diagonal
source `eqW a a` reduces in one step to nothing but the two verdicts `void` and
`integrate (merge a a)`: only `Step.R_eq_refl` and `Step.R_eq_diff` match an
`eqW`-headed root, and inversion on `Step` discards every other rule. This discharges
the `DiagonalDetermined` side condition of the characterization theorem on the real
kernel constructors, closing the only place where the biconditional was applied to KO7
without an explicit side-condition anchor. -/
theorem ko7_diagonal_determined (a : Trace) :
    DiagonalDetermined ko7DiagonalFork a := by
  intro b h
  cases h with
  | R_eq_refl _ => exact Or.inl rfl
  | R_eq_diff _ _ => exact Or.inr rfl

/-- **(Characterization, discharged on the KO7 kernel.)** Local confluence of the full
kernel `Step` at the diagonal `eqW a a` holds if and only if the two verdicts `void`
and `integrate (merge a a)` join. The determinacy side condition is
`ko7_diagonal_determined`; the schema-level biconditional is
`diagonal_localConfluence_iff_verdictsJoin`. -/
theorem ko7_diagonal_localConfluence_iff_verdictsJoin (a : Trace) :
    LocalJoinAt ko7DiagonalFork (eqW a a)
      ↔ DiagonalVerdictsJoin ko7DiagonalFork a :=
  diagonal_localConfluence_iff_verdictsJoin ko7DiagonalFork a (ko7_diagonal_determined a)

#print axioms ko7_diagonal_determined
#print axioms ko7_diagonal_localConfluence_iff_verdictsJoin
