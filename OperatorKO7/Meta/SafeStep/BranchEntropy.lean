...
`verdictBits` is a two-valued branch-presence indicator: it records whether the
verdict count exceeds one. It agrees with `log2` at counts `1` and `2` and
departs from it above `2` (`verdictBits 3 = 1` while `log2 3 > 1`), so it is not
a logarithm. The genuine real-valued functional `Real.logb 2` lives in
`BranchEntropyGeneral.branchEntropy`, which is the public Hartley surface; this
indicator is the internal helper for the two-verdict case, defined by a
decidable comparison so closed instances reduce definitionally.
-/
...
/-- Branch-presence indicator: `0` when a source admits at most one terminal
verdict, `1` when it admits more. Agrees with the Hartley value `log2 n` only at
`n = 1, 2`; the general functional is `BranchEntropyGeneral.branchEntropy`.
Defined by a decidable comparison so the closed instances reduce under
`decide`. -/
