...
/-- **Repair exhaustiveness.** For a determined diagonal, local confluence at `E a a`
holds if and only if the two verdicts join. Every repair that restores diagonal
confluence therefore acts by joining the verdicts, and the guard, delete, and quotient
routes above are exactly the ways to do so: there is no fourth kind of repair. This is
the characterization read as the exhaustive partition of the repair space. -/
theorem diagonal_repair_exhaustive
    (S : UnguardedEqWLike T) (a : T) (hdet : DiagonalDetermined S a) :
    LocalJoinAt S (S.E a a) ↔ DiagonalVerdictsJoin S a :=
  diagonal_localConfluence_iff_verdictsJoin S a hdet
