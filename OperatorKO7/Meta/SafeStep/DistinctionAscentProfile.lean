...
/-- The Distinction (confluence) boundary realizes the six-step shape, sits in the
reflection family, and is stagewise-equivalent to the mechanized dependency-pair
ascent profile. Obtained by the existing comparison theorem, with no new axiom.

Scope: this is a classification, and `Meta/SafeStep/AscentProfileDegeneracy.lean`
measures what it carries. Membership in the class is six true stages plus the
reflection tag (`compatibleWithDp_iff_realizes`), a profile whose six stages are
`True` sits in the same class in both directions (`contentlessToDistinction`,
`distinctionToContentless`), and the comparison hom-sets are subsingletons
(`comparisonWitness_subsingleton`). Witness-level structure is a separate question,
settled by the licensed-ascent objects of
`Meta/LicensedBoundaryCalculus/LicensedAscentObject.lean`, where the distinction and
orientation objects carry this same profile and admit a morphism in neither direction
...
/-- The inverse morphism, so the two profiles are isomorphic in the comparison
category. The isomorphism is the classification: any two realized profiles of the
same family are isomorphic there, including the contentless six-`True` profile
(`comparisonOfRealized`, `contentlessToDistinction`). -/
