...
Carrier module for the forced-output trilemma anchor cited by downstream tooling's
witness-carrier certificate. NO new mathematical content: this module is a
thin definitional carrier that names existing substrate so downstream tooling can
...
- `OperatorKO7.Meta.BoundaryOperator.TypedRefusalCompleteness.TypedRefusalCompleteness_engine_grade`
  (the four-class exhaustiveness statement).
...
/-- Forced-output-trilemma anchor record for downstream consumers.
...
structure ForcedTrilemmaAnchor where
  ...
  /-- Name of the four-class exhaustiveness theorem. -/
...
/-- Canonical anchor cited on every T3 / T4 emission. -/
