...
/-! ## Why the full-capture policy row is theorem-blocked rather than open

The `.fullCapture` row of `hoPolicyRowStatus` carries `.theoremBlocked`. The justification is
collected here, in the one module that sees every component at once. Blocked names a refuted target
interface: the capture-avoidance law is false as stated, the packaged target interface is therefore
uninhabited, the declared counter fails on a concrete beta step, and no direct or variable-constant
higher-order measure orients the duplicating beta family. None of this asserts an unrestricted
higher-order impossibility theorem, and the separately recorded refutation of
`UnqualifiedHigherOrderRewritingLiftClaim` shows
that such an assertion would in fact be false. -/

/-- Collected justification for tagging the full-capture row `.theoremBlocked`. Every field is a
refutation, witness, or no-go. The arbitrary-`c` unconditional result is carried alongside the
retained premise-bearing theorem and the separately named zero-constant theorem. -/
structure FullCaptureRowBlockedJustification : Prop where
  captureAvoidanceLawBlocked : ¬ FullCaptureAvoidanceLaw
  bodyFreshRepairedObligation : FullCaptureAvoidanceLawUpstreamObligation
  targetInterfaceBlocked : ¬ FullCaptureTargetInterface
  exactBoundaryStatus : FullCaptureSemanticsStatus
  universalDirectMeasureNoGo : ∀ M : DirectHOMeasure, ¬ OrientsDuplicatingBeta M
  varConstantFamilyUnboundedInhabitant :
    ∀ c : Nat, ∃ M : VarConstantDirectHOMeasure c, UnboundedRange M
  varConstantUniversalNoGo :
    ∀ (c : Nat) (M : VarConstantDirectHOMeasure c),
      ¬ VarConstantOrientsDuplicatingBeta M
  varConstantConditionalNoGo :
    ∀ (c : Nat) (M : VarConstantDirectHOMeasure c),
      UnboundedRange M → ¬ VarConstantOrientsDuplicatingBeta M
  varConstantZeroUnconditionalNoGo :
    ∀ M : VarConstantDirectHOMeasure 0, ¬ VarConstantOrientsDuplicatingBeta M

/-- The full-capture row is theorem-blocked, with every component proved. -/
theorem full_capture_row_blocked_justification : FullCaptureRowBlockedJustification where
  captureAvoidanceLawBlocked := fullCaptureAvoidanceLaw_blocked
  bodyFreshRepairedObligation := fullCaptureAvoidanceLaw_requiresBodyFreshness
  targetInterfaceBlocked := full_capture_target_interface_blocked
  exactBoundaryStatus := full_capture_exact_boundary_status
  universalDirectMeasureNoGo := no_directHOMeasure_orients_duplicating_beta
  varConstantFamilyUnboundedInhabitant := varConstant_family_has_unbounded_inhabitant
  varConstantUniversalNoGo := fun _ M =>
    no_varConstantDirectHOMeasure_orients_duplicating_beta M
  varConstantConditionalNoGo := fun _ M hUnbounded =>
    no_unbounded_varConstantDirectHOMeasure_orients_duplicating_beta M hUnbounded
  varConstantZeroUnconditionalNoGo := no_varConstantDirectHOMeasure_zero_orients_duplicating_beta

/-- Blocking the full-capture target interface does not yield an unrestricted higher-order
impossibility theorem. The universal nonorientation claim is itself refuted, by the shared-surrogate
orienter. -/
theorem full_capture_block_does_not_yield_unrestricted_impossibility :
    ¬ UnqualifiedHigherOrderRewritingLiftClaim :=
  full_capture_orientation_interface_blocked
