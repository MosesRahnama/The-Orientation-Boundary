...
/-- Stable API alias for beta-free plugging contexts. -/
...
/-- Stable API alias for the decidable classifier catalog. -/
abbrev RewritingDecidableClassifierCatalog : Prop :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.HigherOrderDecidableClassifierCatalog
...
/-- Stable API alias for the proved full-capture counterexample boundary. -/
...
/-- Stable API `isLam` classifier. -/
def is_lam_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.isLam

/-- Stable API binder-free classifier. -/
def binder_free_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.binderFree?

/-- Stable API share-free classifier. -/
def share_free_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.shareFree?

/-- Stable API beta-free classifier. -/
def beta_free_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.betaFree?

/-- Stable API conservative linear classifier. -/
def linear_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.linear?

/-- Stable API conservative DAG/shared classifier. -/
def dag_shared_classifier : RewritingTerm → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.dagShared?

/-- Stable API binder-free context classifier. -/
def binder_free_context_classifier : RewritingContext → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.binderFreeContext?

/-- Stable API beta-free context classifier. -/
def beta_free_context_classifier : RewritingContext → Bool :=
  OperatorKO7.HigherOrderRewritingDecidableClassifiers.betaFreeContext?
...
/-- Stable API projection for the executable classifier catalog. -/
theorem rewriting_decidable_classifier_catalog :
    RewritingDecidableClassifierCatalog :=
...
/-- Stable API `isLam` classifier iff theorem. -/
theorem is_lam_classifier_eq_true_iff
    {t : RewritingTerm} :
    is_lam_classifier t = true ↔ OperatorKO7.HigherOrderRewritingCaptureSubfamilies.IsLam t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_isLam_classifier_eq_true_iff

/-- Stable API binder-free classifier iff theorem. -/
theorem binder_free_classifier_eq_true_iff
    {t : RewritingTerm} :
    binder_free_classifier t = true ↔ RewritingBinderFreeTerm t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_binderFree_classifier_eq_true_iff

/-- Stable API share-free classifier iff theorem. -/
theorem share_free_classifier_eq_true_iff
    {t : RewritingTerm} :
    share_free_classifier t = true ↔ RewritingShareFreeTerm t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_shareFree_classifier_eq_true_iff

/-- Stable API beta-free classifier iff theorem. -/
theorem beta_free_classifier_eq_true_iff
    {t : RewritingTerm} :
    beta_free_classifier t = true ↔ RewritingBetaFreeTerm t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_betaFree_classifier_eq_true_iff

/-- Stable API linear classifier iff theorem. -/
theorem linear_classifier_eq_true_iff
    {t : RewritingTerm} :
    linear_classifier t = true ↔ RewritingLinearTerm t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_linear_classifier_eq_true_iff

/-- Stable API DAG/shared classifier iff theorem. -/
theorem dag_shared_classifier_eq_true_iff
    {t : RewritingTerm} :
    dag_shared_classifier t = true ↔ RewritingDAGSharedTerm t :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_dagShared_classifier_eq_true_iff

/-- Stable API binder-free context classifier iff theorem. -/
theorem binder_free_context_classifier_eq_true_iff
    {c : RewritingContext} :
    binder_free_context_classifier c = true ↔ RewritingBinderFreeContext c :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_binderFreeContext_classifier_eq_true_iff

/-- Stable API beta-free context classifier iff theorem. -/
theorem beta_free_context_classifier_eq_true_iff
    {c : RewritingContext} :
    beta_free_context_classifier c = true ↔ RewritingBetaFreeContext c :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_betaFreeContext_classifier_eq_true_iff

/-- Stable API beta-free classifier closure on the closed fragment. -/
theorem closed_fragment_implies_beta_free_classifier_true
    {t : RewritingTerm} :
    RewritingClosedFragment t → beta_free_classifier t = true :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_closedFragment_implies_betaFree_classifier_true

/-- Stable API binder-free classifier closure on the closed fragment. -/
theorem closed_fragment_implies_binder_free_classifier_true
    {t : RewritingTerm} :
    RewritingClosedFragment t → binder_free_classifier t = true :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_closedFragment_implies_binderFree_classifier_true

/-- Stable API linear classifier closure on share-free closed fragments. -/
theorem share_free_closed_fragment_implies_linear_classifier_true
    {t : RewritingTerm} :
    RewritingClosedFragment t → RewritingShareFreeTerm t → linear_classifier t = true :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_shareFree_closedFragment_implies_linear_classifier_true

/-- Stable API DAG/shared classifier closure on embedded shared terms. -/
theorem embed_shared_term_implies_dag_shared_classifier_true
    (t : SharedTerm) :
    dag_shared_classifier (OperatorKO7.HigherOrderRewritingSyntax.embedSharedTerm t) = true :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_embedSharedTerm_implies_dagShared_classifier_true t

/-- Stable API binder-free plug preservation from the context classifier. -/
theorem binder_free_context_classifier_true_implies_plug_preserves_binder_free
    {c : RewritingContext} {t : RewritingTerm} :
    binder_free_context_classifier c = true → RewritingBinderFreeTerm t →
      RewritingBinderFreeTerm (OperatorKO7.HigherOrderRewritingSyntax.Context.plug c t) :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_binderFreeContext_classifier_true_implies_plug_preserves_binderFree

/-- Stable API beta-free plug preservation from the context classifier. -/
theorem beta_free_context_classifier_true_implies_plug_preserves_beta_free
    {c : RewritingContext} {t : RewritingTerm} :
    beta_free_context_classifier c = true → RewritingBetaFreeTerm t →
      RewritingBetaFreeTerm (OperatorKO7.HigherOrderRewritingSyntax.Context.plug c t) :=
  OperatorKO7.HigherOrderRewritingFinalCatalog.final_catalog_projects_betaFreeContext_classifier_true_implies_plug_preserves_betaFree
...
/-- Stable API projection for the proved full-capture counterexample boundary. -/
...
theorem tree_binder_free_context_closed
    {c : RewritingContext} : RewritingBinderFreeContext c → ∀ {t : RewritingTerm},
      RewritingClosedFragment t → RewritingClosedFragment (OperatorKO7.HigherOrderRewritingSyntax.Context.plug c t) :=
...
theorem beta_step_contextual_closure
    ...
    RewritingBetaStep a b → ∀ context : RewritingContext,
      RewritingContextualBetaStep (OperatorKO7.HigherOrderRewritingSyntax.Context.plug context a)
        (OperatorKO7.HigherOrderRewritingSyntax.Context.plug context b) :=
