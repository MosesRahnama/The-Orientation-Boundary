...
/-! ## Schema-first minimality surface -/

/-- The canonical three-state minimal obstruction crown. -/
def minimal_fork_crown :=
  OperatorKO7.Meta.DistinctionBoundary.MinimalFork.minimal_distinction_boundary_crown

/-- Public three-state lower bound for every finite nonjoinable one-step peak. -/
def minimal_nonjoinable_peak_cardinality :=
  @OperatorKO7.Meta.DistinctionBoundary.MinimalFork.API.distinction_minimal_cardinality

/-- Public exact KO7 local-cone isomorphism to the canonical three-state fork. -/
def ko7_local_cone_is_canonical_fork3 :=
  OperatorKO7.Meta.DistinctionBoundary.MinimalFork.API.distinction_ko7_localCone_is_fork3

/-- Public structural one-bit terminal-support collapse. -/
def canonical_fork_terminal_support_collapse :=
