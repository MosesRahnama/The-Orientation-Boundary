...
This module defines a seven-constructor catalog and assigns each constructor a W1 route, import
class, source tag, status, and support proposition. The support theorems are imported from the
canonical, semantic, generic-DP, and FBI surfaces.

The catalog is exhaustive only for the locally defined `W1MethodRow` type. The permitted-import
theorem follows the support payload in six cases. In the FBI case it uses the separately imported
canonical whole-witness theorem, so it does not derive the permitted import from the FBI support
premise itself.
-/
...
/-- Seven tags used by the local W1 method catalog. -/
...
/-- Proposition combining membership, support, route, and status for every local row tag. -/
...
/-- Every constructor of `W1MethodRow` satisfies the local catalog proposition. -/
...
/-- For every supported row tag, a permitted import of its assigned class is available. The FBI
case uses the canonical imported-whole witness rather than the hypothesis `h`. -/
...
/-- Package the local catalog theorem, the assigned permitted-import theorem, and route separation. -/
...
/-- Construct the certificate from the three preceding local theorems. -/
...
/-- Project the certificate's assigned permitted-import theorem. -/
