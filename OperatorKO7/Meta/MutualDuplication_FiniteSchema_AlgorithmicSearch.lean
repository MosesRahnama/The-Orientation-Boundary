...
searchCycle? checks the predetermined successor edges of the supplied encoded cycle. It is not an arbitrary graph-cycle discovery or SCC-extraction algorithm.
-/
...
/-- Assemble a cycle candidate directly when every required successor edge is defined in the
...
is defined, the executable search returns a witness. -/
...
/-- The executable search fails directly when the explicit finite encoded search space is missing
