...
/-- **Paper 2 Corollary 5.3 (exact witness).** Schema-level operational
incompleteness is obtained *directly* from the projection rank: the returned
value is the evidence bundle itself, with all four fields available to callers.

Relation: `Sys.Step` through the three direct barrier theorems.
Closure: root.
Strategy: full.
Trust: kernel-only.
Scope: every `Sys` equipped with a `ProjectionRank`. -/
def operationalIncompleteness_witness
    {Sys : StepDuplicatingSystem}
    (R : ProjectionRank Sys.toStepDuplicatingSchema) :
    OperationalIncompleteness Sys :=
  OperationalIncompleteness.ofProjectionRank R

/-- **Compatibility corollary of `operationalIncompleteness_witness`.** This
existential form is strictly weaker than the exact witness above: it discards
the evidence bundle and retains only its inhabitation. It is kept for
downstream callers that already depend on this signature and should not be
cited where the exact witness is available. -/
theorem operationalIncompleteness_universal
    {Sys : StepDuplicatingSystem}
    (R : ProjectionRank Sys.toStepDuplicatingSchema) :
    ∃ _ : OperationalIncompleteness Sys, True :=
  ⟨operationalIncompleteness_witness R, trivial⟩
