...
/-- Deferred record: no record bits have yet been committed at stage 2. -/
def DeferredRecord (E : RecordFormationEvent) : Prop :=
...
inductive NonApplicabilityWitness (E : RecordFormationEvent) where
  ...
  | deferredRecord (h : DeferredRecord E)
...
folded into a pure arithmetic proof. -/
...
def unavailableOfNonApplicability
  ...
  | .deferredRecord _ => .unavailable .deferredRecord
...
theorem deferredRecord_not_applicable
    {E : RecordFormationEvent} {T : ℝ}
    (h : DeferredRecord E) :
    ¬ TriggersLandauerBound E T := by
  intro hTrig
...
theorem nonApplicabilityWitness_not_applicable
  ...
  | deferredRecord h => exact deferredRecord_not_applicable h
...
theorem unavailableOfNonApplicability_isUnavailable
  ...
  | deferredRecord _ => exact ⟨.deferredRecord, rfl⟩
