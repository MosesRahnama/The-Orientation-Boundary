...
classifier surface. It remains obligation-scoped: successful checks recover the
...
/-- Executable free-variable occurrence classifier for the current higher-order syntax. -/
...
/-- Executable freshness classifier for binder descent. -/
...
/-- Executable binder-occurrence classifier driven by the current syntax summary. -/
...
@[simp] theorem freeVarOccurs_classifier_eq_true_iff {name : Nat} {t : HOTerm} :
...
@[simp] theorem freeVarOccurs_classifier_eq_false_iff {name : Nat} {t : HOTerm} :
    freeVarOccurs? name t = false ↔ ¬ FreeVarOccurs name t := by
  constructor
  · intro hFalse hOccurs
    have hTrue : freeVarOccurs? name t = true :=
      freeVarOccurs_classifier_eq_true_iff.mpr hOccurs
    rw [hFalse] at hTrue
    cases hTrue
  · intro hNot
    ...
    | true =>
        exfalso
        exact hNot (freeVarOccurs_classifier_eq_true_iff.mp hTrue)

@[simp] theorem freshFor_classifier_eq_true_iff {binderName : Nat} {t : HOTerm} :
    freshFor? binderName t = true ↔ FreshFor binderName t := by
  simp [freshFor?, FreshFor, freeVarOccurs_classifier_eq_false_iff]

@[simp] theorem freshFor_classifier_eq_false_iff {binderName : Nat} {t : HOTerm} :
    freshFor? binderName t = false ↔ ¬ FreshFor binderName t := by
  constructor
  · intro hFalse hFresh
    have hTrue : freshFor? binderName t = true :=
      freshFor_classifier_eq_true_iff.mpr hFresh
    rw [hFalse] at hTrue
    cases hTrue
  · intro hNot
    ...
    | true =>
        exfalso
        exact hNot (freshFor_classifier_eq_true_iff.mp hTrue)
...
@[simp] theorem contextSafeSubstitutionGate_eq_true_iff
  ...
  simp [contextSafeSubstitutionGate, Bool.and_eq_true, and_assoc,
    binderFreeContext_classifier_eq_true_iff, betaFreeContext_classifier_eq_true_iff,
    binderFree_classifier_eq_true_iff, betaFree_classifier_eq_true_iff]
...
theorem captureSafeSubstitutionCheck_succeeds_of_captureSafeSubstitutionObligation
  ...
  have hFreshBool : freshFor? binderName arg = true :=
    freshFor_classifier_eq_true_iff.mpr h.binderAware.freshArgument
  have hArgBinderBool : binderFree? arg = true :=
    binderFree_classifier_eq_true_iff.mpr h.argumentBinderFree
  have hArgShareBool : shareFree? arg = true :=
    shareFree_classifier_eq_true_iff.mpr h.argumentShareFree
  have hBodyBinderBool : binderFree? body = true :=
    binderFree_classifier_eq_true_iff.mpr h.bodyBinderFree
  have hBodyShareBool : shareFree? body = true :=
    shareFree_classifier_eq_true_iff.mpr h.bodyShareFree
...
theorem contextSafeSubstitutionCheck_succeeds_of_contextSafeSubstitutionObligation
  ...
  have hBinderContextBool : binderFreeContext? c = true :=
    binderFreeContext_classifier_eq_true_iff.mpr h.binderFreeContext
  have hBetaContextBool : betaFreeContext? c = true :=
    betaFreeContext_classifier_eq_true_iff.mpr h.betaFreeContext
  have hBinderTermBool : binderFree? t = true :=
    binderFree_classifier_eq_true_iff.mpr h.termBinderFree
  have hBetaTermBool : betaFree? t = true :=
    betaFree_classifier_eq_true_iff.mpr h.termBetaFree
...
/-- Successful context checks preserve binder-freeness under plugging. -/
theorem contextSafeSubstitutionCheck_success_preserves_binderFree_plug
    {c : Context} {t : HOTerm} :
    (∃ cert, contextSafeSubstitutionCheck c t = some cert) →
      BinderFreeHOTerm (Context.plug c t) := by
  intro hSuccess
  exact contextSafeSubstitutionObligation_plug_binder_free
    (contextSafeSubstitutionCheck_success_implies_obligation hSuccess)

/-- Successful context checks preserve beta-freeness under plugging. -/
theorem contextSafeSubstitutionCheck_success_preserves_betaFree_plug
    {c : Context} {t : HOTerm} :
    (∃ cert, contextSafeSubstitutionCheck c t = some cert) →
      BetaFreeHOTerm (Context.plug c t) := by
  intro hSuccess
  exact contextSafeSubstitutionObligation_plug_beta_free
    (contextSafeSubstitutionCheck_success_implies_obligation hSuccess)

/-- The executable capture-decision layer is sound and complete on its exact obligation surface. -/
...
/-- The executable layer records the proved full-capture counterexample boundary. -/
theorem capture_decision_full_capture_semantics_exact_boundary :
...
structure CaptureDecisionExactBoundaryStatus : Prop where
  obligationScoped :
    ∀ {name binderName : Nat} {arg body : HOTerm},
      (∃ cert, captureSafeSubstitutionCheck name binderName arg body = some cert) →
        CaptureSafeSubstitutionObligation name binderName arg body
  fullCaptureSemanticsExactBoundary : FullCaptureSemanticsStatus

/-- Canonical boundary marker for the current executable decision layer. -/
theorem capture_decision_exact_boundary_status : CaptureDecisionExactBoundaryStatus := by
  refine ⟨?_, ?_⟩
  · intro name binderName arg body hSuccess
    exact captureSafeSubstitutionCheck_success_is_obligation_scoped hSuccess
  · exact capture_decision_full_capture_semantics_exact_boundary
...
structure HigherOrderCaptureDecidableCatalog : Prop where
  ...
  contextSafeCheckerBinderFreePlugPreservation :
    ∀ {c : Context} {t : HOTerm},
      (∃ cert, contextSafeSubstitutionCheck c t = some cert) →
        BinderFreeHOTerm (Context.plug c t)
  contextSafeCheckerBetaFreePlugPreservation :
    ∀ {c : Context} {t : HOTerm},
      (∃ cert, contextSafeSubstitutionCheck c t = some cert) →
        BetaFreeHOTerm (Context.plug c t)
  boundaryStatus : CaptureDecisionExactBoundaryStatus
...
theorem higher_order_capture_decidable_catalog :
  ...
  · intro name t
    exact freeVarOccurs_classifier_eq_true_iff
  · intro binderName t
    exact freshFor_classifier_eq_true_iff
  ...
  · intro c t hSuccess
    exact contextSafeSubstitutionCheck_success_preserves_binderFree_plug hSuccess
  · intro c t hSuccess
    exact contextSafeSubstitutionCheck_success_preserves_betaFree_plug hSuccess
  · exact capture_decision_exact_boundary_status
