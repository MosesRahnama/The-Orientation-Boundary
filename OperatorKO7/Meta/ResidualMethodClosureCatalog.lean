...
Terminal classification of all twenty-five rows is weaker than closure of all
twenty-five rows.  Two of the five terminal statuses, `blocked` and
`licensedEscape`, are not closure by a Lean theorem of this development: the
first records a standing obstruction, the second records closure available only
under an external soundness license.  The certificate therefore also exposes an
explicit non-overclaim boundary, `notUniversallyClosed`, which keeps "every row
is terminally classified" apart from "every row is closed".
-/
...
/-- A row is *not internally closed* when its status is `blocked`, a standing
obstruction, or `licensedEscape`, closure available only under an external
soundness license.  Neither status is closure by a Lean theorem of this
development. -/
def ResidualMethodClosureRowNotInternallyClosed
    (row : ResidualMethodClosureCatalogRow) : Prop :=
  residualMethodClosureCatalogRowStatus row = .blocked ∨
    residualMethodClosureCatalogRowStatus row = .licensedEscape

/-- The explicit non-overclaim boundary of the catalog: at least one row is not
internally closed, and it is false that every row is closed by a Lean theorem.
This is the exact statement that the twenty-five-row inventory is terminally
classified without being universally closed. -/
abbrev ResidualMethodClosureNotUniversallyClosed : Prop :=
  (∃ row : ResidualMethodClosureCatalogRow,
      row ∈ residualMethodClosureCatalogRows ∧
        ResidualMethodClosureRowNotInternallyClosed row)
  ∧ ¬ (∀ row : ResidualMethodClosureCatalogRow,
        residualMethodClosureCatalogRowStatus row = .closedByLeanTheorem)

/-- Non-vacuity witness for the `blocked` disjunct: generic dependency-pair
direct pair extraction is a standing obstruction. -/
theorem residualMethodClosureCatalog_exhibits_blocked :
    residualMethodClosureCatalogRowStatus
      .genericDPDirectPairExtraction = .blocked := rfl

/-- Non-vacuity witness for the `licensedEscape` disjunct: the full arctic
matrix route escapes only under an external license. -/
theorem residualMethodClosureCatalog_exhibits_licensedEscape :
    residualMethodClosureCatalogRowStatus
      .matrixArcticFullLicensedEscape = .licensedEscape := rfl

/-- The catalog is terminally classified but not universally closed. -/
theorem residualMethodClosureCatalog_not_universally_closed :
    ResidualMethodClosureNotUniversallyClosed := by
  refine ⟨⟨.genericDPDirectPairExtraction, ?_,
    Or.inl residualMethodClosureCatalog_exhibits_blocked⟩, ?_⟩
  · decide
  · intro h
    exact absurd (h .genericDPDirectPairExtraction) (by decide)
  ...
  allRowsTerminal :
    ∀ row : ResidualMethodClosureCatalogRow,
      residualMethodClosureCatalogRowStatus row = .blocked ∨
        ...
        residualMethodClosureCatalogRowStatus row = .licensedEscape ∨
        residualMethodClosureCatalogRowStatus row = .certifiedSuccess
  notUniversallyClosed : ResidualMethodClosureNotUniversallyClosed

/-- Terminally classified residual-method certificate.  Every row carries a
terminal status and its owning theorem support; the `notUniversallyClosed`
field records that terminal classification is not universal closure. -/
theorem residualMethodClosureCertificate :
  ...
  notUniversallyClosed := residualMethodClosureCatalog_not_universally_closed
...
/-- Certificate projection to the explicit non-overclaim boundary: the
twenty-five-row inventory is not universally closed. -/
theorem residualMethodClosureCertificate_projects_notUniversallyClosed :
    ResidualMethodClosureNotUniversallyClosed :=
