...
def leanModuleOf : RDRSMethodFamily → String
  ...
  -- Coefficient-weighted KBO
  | .subtermCoefficientKBO
      => "Meta.KBO_SubtermCoefficient"
...
def theoremIdOf : RDRSMethodFamily → String
  ...
  -- Blocked: coefficient-weighted variable condition
  | .subtermCoefficientKBO
      => "OperatorKO7.KBOSubtermCoefficient.subtermCoefficientKBO_no_ko7_rec_succ"
  -- Projection escapes
  | .dpSubtermCriterion | .dpArgumentFiltering
  ...
  | .acRPO | .rpoModuloPermutation | .cichonSlowGrowing
...
def frictionNoteOf : RDRSMethodFamily → String
  | .subtermCoefficientKBO
      => "Subterm coefficient KBO: the variable condition is coefficient-weighted, so the payload count rises from recur2 to wrap1 + wrap2 * recur2. No coefficient assignment meets it; the row is blocked by subtermCoefficientKBO_no_ko7_rec_succ."
...
theorem blockedFamilies_length : blockedFamilies.length = 21 := by decide

theorem projectionEscapeFamilies_length : projectionEscapeFamilies.length = 8 := by decide

theorem constructionEscapeFamilies_length : constructionEscapeFamilies.length = 15 := by decide
...
structure CoverageSeedClosed : Prop where
  ...
  blockedCount       : blockedFamilies.length = 21
  projectionCount    : projectionEscapeFamilies.length = 8
  constructionCount  : constructionEscapeFamilies.length = 15
