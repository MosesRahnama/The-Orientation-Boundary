...
* `relationalCategory` -- the deterministic endofunction presentation fails,
  but the terminating reflexive-transitive rewrite category exists and carries
  the equality fork (`conditional_escape`).
...
def mechanismOf : SemanticStructuralRow → Mechanism
  ...
  | .categoricalToposTermination      => .relationalCategory
...
theorem categoricalToposTermination_conditional_escape :
    localStatusOf .categoricalToposTermination = .conditional_escape := rfl
