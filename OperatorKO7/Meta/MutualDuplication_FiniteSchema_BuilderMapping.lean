...
This module packages the H3 finite-cycle builder as a search-certificate
transport. The certificate contains the local successor-edge family as
caller-supplied proof data. Algorithmic extraction of those edges requires a
separate implementation and soundness theorem.
-/
