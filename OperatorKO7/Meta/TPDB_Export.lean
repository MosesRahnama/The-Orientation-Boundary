...
/-!
## Free two-rule recursor export

The free recursor `freeRecursorTRS` of
`OperatorKO7.Meta.Methods.OrientationClosure.DependencyPairSoundness`
(`recur(b, s, zero) → b`, `recur(b, s, succ(n)) → wrap(s, recur(b, s, n))`),
rendered in the same TPDB syntax. Its termination is proved Lean-side as
`freeRecursorTRS_terminating` via the subterm criterion.
-/

/-- Variable shorthands for the exported free recursor. -/
def tb : TpdbTerm := .var "b"
def ts : TpdbTerm := .var "s"
def tn : TpdbTerm := .var "n"

/-- Constructor / function shorthands for the free recursor signature. -/
def tzero : TpdbTerm := .app "zero" []
def tsucc (t : TpdbTerm) : TpdbTerm := .app "succ" [t]
def twrap (a b : TpdbTerm) : TpdbTerm := .app "wrap" [a, b]
def trecur (a b c : TpdbTerm) : TpdbTerm := .app "recur" [a, b, c]

/-- The two free-recursor rules in exported order. -/
def freeRecursorTpdbRules : List TpdbRule :=
  [ ⟨trecur tb ts tzero, tb⟩
  , ⟨trecur tb ts (tsucc tn), twrap ts (trecur tb ts tn)⟩
  ]

/-- Concrete TPDB / TTT2 text for the free two-rule recursor. -/
def free_recursor_tpdb : String :=
  "(VAR b s n)\n" ++
  "(RULES\n" ++
  renderRuleBlock freeRecursorTpdbRules ++
  ")\n"

/-- Embedded reference literal for the expected free-recursor TPDB rendering. -/
def free_recursor_tpdb_embedded_reference : String :=
  "(VAR b s n)\n" ++
  "(RULES\n" ++
  "  recur(b, s, zero) -> b\n" ++
...
/-- Historical compatibility alias. The name does not imply that Lean reads the external file. -/
abbrev free_recursor_tpdb_artifact_text : String :=
  free_recursor_tpdb_embedded_reference

/-- The renderer output is definitionally equal to the embedded reference literal. -/
theorem free_recursor_tpdb_matches_embedded_reference :
    free_recursor_tpdb = free_recursor_tpdb_embedded_reference := by
  rfl

/-- External artifact parity is checked outside Lean. -/
theorem free_recursor_tpdb_matches_artifact_text :
    free_recursor_tpdb = free_recursor_tpdb_artifact_text :=
  free_recursor_tpdb_matches_embedded_reference

/-- The declared free-recursor TPDB rule list has length two. -/
theorem free_recursor_tpdb_rule_count :
    freeRecursorTpdbRules.length = 2 := rfl
