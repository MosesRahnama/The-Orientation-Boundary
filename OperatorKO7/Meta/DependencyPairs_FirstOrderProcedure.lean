...
/-!
# Procedure-Level Frontend for Generic First-Order Extraction
...
at each use site, callers can package an internal first-order TRS / DP procedure once as a
single structure. The same extracted-node, call-graph, SCC-search, and contradiction
surface is then reexported from that procedure object directly.
-/

namespace OperatorKO7.DependencyPairsFragment

/-- Minimal finite first-order rule procedure packaged as one object. -/
structure FiniteFirstOrderProcedure (σ ν : Type) [DecidableEq σ] where
  Rule : Type
  rules : Array Rule
  lhs : Rule → FOTerm σ ν
  rhs : Rule → FOTerm σ ν

namespace FiniteFirstOrderProcedure

variable {σ ν : Type} [DecidableEq σ] (E : FiniteFirstOrderProcedure σ ν)

/-- Defined heads of the packaged procedure. -/
abbrev definedHeads : Finset σ :=
  foDefinedHeadsOf E.rules E.lhs

/-- Extracted dependency-pair nodes of the packaged procedure. -/
abbrev extractedNodes : Array (ExtractedRuleFrontendNode E.Rule σ) :=
  extractRuleFrontendNodes E.rules E.lhs E.rhs

/-- Array-backed extracted call graph of the packaged procedure. -/
abbrev extractedCallGraph : FiniteExtractedCallGraph σ :=
  foExtractedCallGraphOf E.rules E.lhs E.rhs

/-- Direct SCC search on the packaged procedure. -/
...
/-- SCC existence predicate on the packaged procedure. -/
...
theorem hasNontrivialSCC_iff_exists_findNontrivialSCCPair? :
  ...
  simpa [FiniteFirstOrderProcedure.HasNontrivialSCC, FiniteFirstOrderProcedure.findNontrivialSCCPair?,
    FiniteFirstOrderProcedure.extractedCallGraph] using
...
/-- Standard SCC witness recovered directly from the packaged procedure. -/
...
theorem not_globalOrients_of_source_le_target_of_findNontrivialSCCPair?
  ...
  simpa [FiniteFirstOrderProcedure.findNontrivialSCCPair?, FiniteFirstOrderProcedure.extractedCallGraph]
    using
...
theorem not_globalOrients_of_source_le_target_of_hasNontrivialSCC
    ...
    ¬ GlobalOrients E.extractedCallGraph.toFiniteCallGraph.Edge m (· < ·) := by
  simpa [FiniteFirstOrderProcedure.HasNontrivialSCC, FiniteFirstOrderProcedure.toSCCCycle,
    FiniteFirstOrderProcedure.extractedCallGraph] using
    (not_globalOrients_of_source_le_target_of_hasNontrivialSCCOf
      E.rules E.lhs E.rhs h hge)

end FiniteFirstOrderProcedure

/-- Typeclass view exposing a finite first-order rule procedure from an internal system type. -/
...
/-- Canonical packaged procedure induced by the typeclass view. -/
def toFiniteFirstOrderProcedure (E : ε) : FiniteFirstOrderProcedure σ ν where
...
/-- Defined heads exposed directly from a viewed procedure value. -/
abbrev definedHeads (E : ε) : Finset σ :=
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).definedHeads

/-- Extracted nodes exposed directly from a viewed procedure value. -/
abbrev extractedNodes (E : ε) :
    Array (ExtractedRuleFrontendNode (HasFiniteFirstOrderView.Rule (ε := ε) (σ := σ) (ν := ν)) σ) :=
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).extractedNodes

/-- Extracted call graph exposed directly from a viewed procedure value. -/
abbrev extractedCallGraph (E : ε) : FiniteExtractedCallGraph σ :=
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).extractedCallGraph

/-- Direct SCC search exposed from a viewed procedure value. -/
noncomputable abbrev findNontrivialSCCPair? (E : ε) :
  ...
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).findNontrivialSCCPair?

/-- SCC existence predicate exposed from a viewed procedure value. -/
abbrev HasNontrivialSCC (E : ε) : Prop :=
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).HasNontrivialSCC

/-- Standard SCC witness exposed from a viewed procedure value. -/
noncomputable abbrev toSCCCycle (E : ε)
    (h : (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).HasNontrivialSCC) :
    SCCCycle (extractedCallGraph (ε := ε) (σ := σ) (ν := ν) E).Node :=
  (toFiniteFirstOrderProcedure (ε := ε) (σ := σ) (ν := ν) E).toSCCCycle h
...
/-- Raw finite first-order procedure view using an internal term syntax stored in the class. -/
...
/-- Any raw first-order procedure view induces the canonical `FOTerm`-based view automatically. -/
