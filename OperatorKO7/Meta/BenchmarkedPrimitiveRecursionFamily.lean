...
/-!
# Finite classification of the benchmarked primitive-recursion family

This module classifies the six configurations generated by the two-rule
recursor pattern on the `RecCore` signature. It does not classify arbitrary
...
Here, "global" and "minimum" refer only to this six-member family and to the
absence of a direct additive whole-term witness.
-/
...
/-- Arbitrary natural-valued decreasing rank over the original relation. The
type does not record where the rank was obtained. -/
def HasImportedWholeWitness (cfg : PRCConfig) : Prop :=
  ∃ μ : RecCoreTerm → Nat,
    ∀ {a b : RecCoreTerm}, FamilyStep cfg a b → μ b < μ a

/-- Natural-valued decreasing rank over the call-focused relation, which keeps
the duplicating rule's outer `app` wrapper. -/
...
/-- Every structurally complete member other than `fullDuplicating` has a
direct additive whole-term witness. -/
...
/-- Within this six-member family, `fullDuplicating` is the unique structurally
complete configuration without a direct additive whole-term witness; it still
has the two broader witness forms defined above. -/
