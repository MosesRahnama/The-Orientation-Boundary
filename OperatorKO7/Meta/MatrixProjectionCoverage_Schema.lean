...
/-!
# Fixed-row and row-sum matrix projection corollaries

This module specializes imported matrix barrier theorems to two projection patterns:

- a fixed tracked row, using `MatrixBarrierD`;
- an all-ones weighted functional, using `MatrixBarrierFunctional`.
-/
